...
class ErrorSource(Enum):
    BSI = "BSI Technical Guideline TR-02103"
    RFC5280 = "RFC5280"
    TRUSTPOINT = "Trustpoint specified constraint."


class CertificateError(Enum):
    # error msg, code, source
    INVALID_VERSION = (
        "Certificate version is not v3.",
        "invalid_version",
        ErrorSource.TRUSTPOINT,
    )
    SERIAL_NUMBER_0 = (
        "Certificate serial number is 0.",
        "serial_number_0",
        ErrorSource.RFC5280,
    )
    ...
    SHORT_SERIAL_NUMBER = (
        "Certificate serial number is shorter than 20 bytes.",
        "short_serial_number",
    )
    EXT_ISSUER_ALTERNATIVE_NAME_CRITICAL = (
        "The Issuer Alternative Name extension of the CA certificate is marked as critical.",
        "ext_key_usage_non_critical",
    )
    EXT_KEY_USAGE_MISSING = (
        "CA certificate does not contain the Key Usage extension.",
        "ext_key_usage_missing",
    )
    EXT_KEY_USAGE_NON_CRITICAL = (
        "The Key Usage extension of the CA certificate is marked as non-critical.",
        "ext_key_usage_non_critical",
    )
    EXT_POLICY_MAPPING_NON_CRITICAL = (
        "The Policy Mappings extension of the CA certificate is marked as non-critical.",
    ...
    EXT_AUTHORITY_KEY_ID_MISSING = (
        "Certificate does not contain the Authority Key Identifier extension.",
        "ext_authority_key_id_missing",
    )
    EXT_AUTHORITY_KEY_ID_CRITICAL = (
        "The Authority Key Identifier extension is marked as critical.",
        "authority_key_id_critical",
    )
    EXT_SUBJECT_KEY_ID_CRITICAL = (
        "The Subject Key Identifier extension is marked as critical.",
        "subject_key_id_critical",
    )
    EXT_BASIC_CONSTRAINTS_MISSING = (
        "CA certificate does not have a Basic Constraints extension.",
        "ext_basic_constraints_missing",
    )
    EXT_BASIC_CONSTRAINTS_NON_CRITICAL = (
        "The Basic Constraints extension of the CA certificate is marked as non-critical.",
        "ext_basic_constraints_non_critical",
    )
    EXT_BASIC_CONSTRAINTS_NON_CA = (
        "CA certificate has a Basic Constraints extension stating that the certificate is not a CA.",
        "ext_basic_constraints_non_ca",
    )
    ...
    EXT_SUBJECT_KEY_ID_MISSING = (
        "Certificate does not contain the Subject Key Identifier extension.",
        "ext_authority_key_id_missing",
    )
    EXT_POLICY_CONSTRAINTS_NON_CRITICAL = (
        "The Policy Constraints extension of the CA certificate is marked as non-critical.",
        "ext_policy_constraints_non_critical",
    )
    EXT_INHIBIT_ANY_POLICY_NON_CRITICAL = (
        "The Inhibit Any Policy extension of the CA certificate is marked as non-critical.",
        "ext_policy_mappings_non_critical",
    )
    ...
    EXT_AUTHORITY_KEY_ID_KEY_ID_MISSING = (
        "The Authority Key Identifier Extension of the CA certificate does not contain the Key Identifier field.",
        "ext_authority_key_id_missing",
    )
    ...
    def _check_authority_key_identifier(self) -> None:
        try:
            authority_key_identifier = (
                self.certificate.extensions.get_extension_for_class(
...
class CaCertificateValidator(CertificateValidator):
    ...
    def _check_key_usage(self) -> None:
        ...
        if not (
            ...
            or key_usage.value.digital_signature
        ):
    ...
    def _check_certificate_policies(self) -> None:
        try:
            policy_constraints = self.certificate.extensions.get_extension_for_class(
    ...
    def _check_policy_mappings(self) -> None:
        try:
            policy_mappings = self.certificate.extensions.get_extension_for_oid(
                x509.oid.ExtensionOID.POLICY_MAPPINGS
            )
        except x509.ExtensionNotFound:
            return
    ...
    def _check_subject_alternative_name(self) -> None:
        try:
            subject_alternative_name = (
                self.certificate.extensions.get_extension_for_oid(
    ...
    def _check_basic_constraints(self) -> None:
        try:
            basic_constraints = self.certificate.extensions.get_extension_for_class(
    ...
    def _check_inhibit_any_policy(self) -> None:
        try:
            inhibit_any_policy = self.certificate.extensions.get_extension_for_class(
...
#
#     def __init_self, certificate: x509.Certificate, additional_certificates: list[x509.Certificate]) -> None:
#         self._certificate = certificate
