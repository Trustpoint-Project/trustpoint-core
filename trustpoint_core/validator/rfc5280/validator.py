...
class Validation(ABC):
    ...
    def __init__(self) -> None:
        ...
        self._components: list[
            Validation
        ] = []  # List of child validations (leafs or composites)
    ...
    def validate_ipaddress(value: str, context: str) -> tuple[bool, list[str]]:
        ...
        try:
            ...
            try:
                ...
                errors.append(
                    f"{context} iPAddress contains a network address: {network}. Networks are not standard."
                )
            except ValueError:
        ...
        try:
            ...
            if signature_algorithm_oid.dotted_string not in self.supported_algorithms:
                algorithm_name = getattr(
                    signature_algorithm_oid, "name", "Unknown Algorithm"
                )
...
class IssuerValidation(Validation):
    ...
    def validate(self, cert: Certificate) -> bool:
        ...
        try:
            ...
            if not issuer:
                self.log_error(
                    "Issuer field is empty. It must contain a non-empty distinguished name (DN)."
                )
                return False
            ...
            for attribute in issuer:
                ...
                if not value:
                    self.log_error(
                        f"Issuer attribute {oid.dotted_string} has an empty value."
                    )
                    result = False
...
class ValidityValidation(Validation):
    ...
    def validate(self, cert: Certificate) -> bool:
        ...
        try:
            ...
            if cert_not_after == datetime(
                9999, 12, 31, 23, 59, 59, tzinfo=timezone.utc
            ):
                self.log_warning(
                    "Certificate has no well-defined expiration date (notAfter set to 99991231235959Z)."
                )
        ...
        try:
            ...
            try:
                ...
                if not list(subject) and not subject_alt_name.critical:
                    self.log_error(
                        "If the subject field is empty, the subjectAltName extension must be critical."
                    )
                    result = False
            except Exception:  # noqa: BLE001
                if not list(subject):
                    self.log_error(
                        "Subject field is empty, and no subjectAltName extension is present."
                    )
                    result = False
            ...
            for attribute in subject:
                ...
                if not value:
                    self.log_error(
                        f"Subject attribute {oid.dotted_string} has an empty value."
                    )
                    result = False
...
class SubjectPublicKeyInfoValidation(Validation):
    ...
    def __init__(
        self, supported_algorithms: set[type[PublicKey]] | None = None
    ) -> None:
    ...
    def validate(self, cert: Certificate) -> bool:
        ...
        try:
            ...
            self.log_error(
                f"Unexpected error during Subject Public Key Info validation: {e}"
            )
            result = False
    ...
    def _validate_rsa_key(self, key: rsa.RSAPublicKey) -> bool:
        ...
        try:
            ...
            if key_size < self.MIN_RSA_KEY_SIZE:
                self.log_error(
                    f"RSA key size too small: {key_size} bits. Minimum recommended size is 2048 bits."
                )
                return False
        except Exception as e:  # noqa: BLE001
        ...
        try:
            ...
            if version == 1:
                if issuer_unique_id is not None or subject_unique_id is not None:
                    self.log_error(
                        "Unique identifiers must not appear in version 1 certificates."
                    )
                    result = False
            elif version in [2, 3]:
                if issuer_unique_id is not None or subject_unique_id is not None:
                    self.log_warning(
                        "Unique identifiers are present, but their use is discouraged by the profile."
                    )
            else:
            ...
            self.log_error(
                f"Unexpected error during Unique Identifiers validation: {e}"
            )
            result = False
        ...
        try:
            ...
            if aki_ext is None:
                if not is_self_signed:
                    ...
                    self.log_warning(
                        "AKI extension is missing in a self-signed certificate, which is permitted."
                    )
                return result
            ...
            if aki_ext.critical:
                self.log_error(
                    "Authority Key Identifier (AKI) extension must be marked as non-critical."
                )
                result = False
            ...
            if aki.key_identifier is None:
                if not is_self_signed:
                    self.log_error(
                        "keyIdentifier field in AKI extension is missing in a non-self-signed certificate."
                    )
                    result = False
                else:
                    self.log_warning(
            ...
            self.log_error(
                f"Unexpected error during Authority Key Identifier validation: {e}"
            )
            result = False
...
class SubjectKeyIdentifierValidation(Validation):
    ...
    def _map_signature_oid_to_hash_algorithm(
        self, sig_oid: ObjectIdentifier
    ...
    def _compute_key_identifier(
        self, public_key: PublicKey, hash_algorithm: HashAlgorithm
    ) -> bytes:
    ...
    def validate(self, cert: Certificate) -> bool:
        ...
        try:
            ...
            # Compute the SKI
            computed_ski = self._compute_key_identifier(
                cert.public_key(), hash_algorithm
            )
            ...
            if ski_ext.value.digest != computed_ski:
                self.log_warning(
                    "Computed SKI does not match the certificate's SKI value."
                )
                result = False
...
class KeyUsageValidation(Validation):
    ...
    def _validate_ca_specifics(self, key_usage: KeyUsage) -> bool:
        ...
        if not key_usage.key_cert_sign:
            self.log_error(
                "CA certificate must have the keyCertSign bit set in the Key Usage extension."
            )
            result = False
        return result
...
class CertificatePoliciesValidation(Validation):
    ...
    def _get_certificate_policies_extension(
        self, cert: Certificate
    ) -> Extension[CertificatePolicies] | None:
        ...
        try:
            ...
            if self.require_extension:
                self.log_error(
                    "Certificate Policies extension is required but missing."
                )
            else:
                self.log_warning(
                    "Certificate Policies extension is missing. Its presence depends on usage."
                )
            return None

    def _validate_extension_criticality(
        self, cert_policies_ext: Extension[CertificatePolicies]
    ) -> bool:
    ...
    def _validate_policy_identifiers(self, cert_policies: CertificatePolicies) -> bool:
        ...
        for policy in cert_policies:
            if policy.policy_identifier in seen_oids:
                self.log_error(
                    f"Duplicate policy identifier found: {policy.policy_identifier}."
                )
                result = False
            ...
            if policy.policy_qualifiers:
                for qualifier in policy.policy_qualifiers:
                    if hasattr(qualifier, "explicit_text"):
                        result = (
                            self._validate_explicit_text(qualifier.explicit_text)
                            and result
                        )
    ...
    def validate(self, cert: Certificate) -> bool:
        ...
        try:
            ...
            if san_ext is None:
                if self.require_extension:
                    self.log_error(
                        "Subject Alternative Name (SAN) extension is required but missing."
                    )
                    result = False
                return result
...
class IssuerAlternativeNameValidation(Validation):
    ...
    def validate(self, cert: Certificate) -> bool:
        ...
        try:
            ...
            try:
                ian_ext = cert.extensions.get_extension_for_oid(
            ...
            if ian_ext is None:
                if self.require_extension:
                    self.log_error(
                        "Issuer Alternative Name (IAN) extension is required but missing."
                    )
                    result = False
                return result
            ...
            if ian_ext.critical:
                self.log_warning(
                    "Issuer Alternative Name (IAN) extension should be marked as non-critical."
                )
...
class SubjectDirectoryAttributesValidation(Validation):
    ...
    def validate(self, cert: Certificate) -> bool:
        ...
        try:
            ...
            try:
                sda_ext = cert.extensions.get_extension_for_oid(
            ...
            if sda_ext is None:
                if self.require_extension:
                    self.log_error(
                        "Subject Directory Attributes extension is required but missing."
                    )
                    result = False
                return result
            ...
            if sda_ext.critical:
                self.log_error(
                    "Subject Directory Attributes extension must be marked as non-critical."
                )
                result = False
            ...
            if not attributes:
                self.log_error(
                    "Subject Directory Attributes extension is present but contains no attributes."
                )
                result = False

        except Exception as e:  # noqa: BLE001
            self.log_error(
                f"Unexpected error during Subject Directory Attributes validation: {e}"
            )
            result = False
...
class BasicConstraintsValidation(Validation):
    ...
    def _get_basic_constraints_extension(
        self, cert: Certificate
    ) -> Extension[BasicConstraints] | None:
    ...
    def _validate_criticality(self, ext: Extension[BasicConstraints]) -> bool:
        ...
        if self.is_ca and not ext.critical:
            self.log_error(
                "Basic Constraints extension must be marked as critical in CA certificates."
            )
            return False
        return True
    ...
    def _validate_ca_boolean(self, basic_constraints: BasicConstraints) -> bool:
        ...
        if self.is_ca and not ca:
            self.log_error(
                "CA certificate must assert the cA boolean in the Basic Constraints extension."
            )
            return False
    ...
    def _validate_path_len_constraint(
        ...
        if path_len is not None:
            if not basic_constraints.ca:
                self.log_error(
                    "pathLenConstraint must not be present unless the cA boolean is asserted."
                )
                return False
            if path_len < 0:
                self.log_error(
                    "pathLenConstraint must be greater than or equal to zero."
                )
                return False
        return True

    def _validate_key_usage(
        self, cert: Certificate, basic_constraints: BasicConstraints
    ) -> bool:
        ...
        try:
            ...
            self.log_warning(
                "Key Usage extension is missing. Cannot validate keyCertSign bit."
            )
...
class NameConstraintsValidation(Validation):
    ...
    def validate(self, cert: Certificate) -> bool:
        ...
        try:
            ...
            try:
                name_constraints_ext = cert.extensions.get_extension_for_oid(
            ...
            if name_constraints_ext is None:
                if self.require_extension:
                    self.log_error(
                        "Name Constraints extension is required but missing."
                    )
                    result = False
                return result
...
class PolicyConstraintsValidation(Validation):
    ...
    def validate(self, cert: Certificate) -> bool:
        ...
        try:
            ...
            try:
                policy_constraints_ext = cert.extensions.get_extension_for_oid(
            ...
            if policy_constraints_ext is None:
                if self.require_extension:
                    self.log_error(
                        "Policy Constraints extension is required but missing."
                    )
                    result = False
                return result
            ...
            if not policy_constraints_ext.critical:
                self.log_error(
                    "Policy Constraints extension must be marked as critical."
                )
                result = False
            ...
            self.log_error(
                f"Unexpected error during Policy Constraints validation: {e}"
            )
            result = False
        ...
        try:
            ...
            if eku_ext is None:
                if not self.is_ca and self.required_purposes:
                    self.log_error(
                        "Extended Key Usage extension is required but missing."
                    )
                    result = False
                return result
            ...
            if ExtendedKeyUsageOID.ANY_EXTENDED_KEY_USAGE in eku_values:
                if eku_ext.critical:
                    self.log_error(
                        "anyExtendedKeyUsage is present, but the EKU extension is marked as critical."
                    )
                    result = False
                self.log_warning(
                    "anyExtendedKeyUsage is present. No restrictions on key purposes."
                )
            ...
            if self.required_purposes:
                for purpose in self.required_purposes:
                    if purpose not in eku_values:
                        self.log_error(
                            f"Required key purpose {purpose} is missing from EKU."
                        )
                        result = False

            # Cross-validation with Key Usage
            result = (
                self._validate_consistency_with_key_usage(cert, eku_values) and result
            )

        except Exception as e:  # noqa: BLE001
            self.log_error(
                f"Unexpected error during Extended Key Usage validation: {e}"
            )
            result = False

        return result

    def _validate_consistency_with_key_usage(
        self, cert: Certificate, eku_values: ExtendedKeyUsage
    ) -> bool:
        ...
        try:
            ...
            self.log_warning(
                "Key Usage extension is missing. Cannot validate consistency with EKU."
            )

        return result

    def _is_key_usage_consistent(
        self, eku: ObjectIdentifier, key_usage: KeyUsage
    ) -> bool:
        ...
        eku_to_check: dict[ObjectIdentifier, Callable[[], bool]] = {
            ...
            ExtendedKeyUsageOID.CLIENT_AUTH: lambda: key_usage.digital_signature
            or key_usage.key_agreement,
        ...
        if eku in eku_to_check and not eku_to_check[eku]():
            self.log_error(
                f"Key Usage is inconsistent with {eku.dotted_string} in EKU."
            )
            return False
...
class CRLDistributionPointsValidation(Validation):
    ...
    def validate(self, cert: Certificate) -> bool:
        ...
        try:
            ...
            if crl_ext.critical:
                self.log_warning(
                    "CRL Distribution Points extension should be non-critical as recommended by RFC 5280."
                )
                result = False

        except Exception as e:  # noqa: BLE001
            self.log_error(
                f"Unexpected error during CRL Distribution Points validation: {e}"
            )
            result = False

        return result

    def _get_crl_extension(
        self, cert: Certificate
    ) -> Extension[CRLDistributionPoints] | None:
    ...
    def _validate_distribution_point(self, dp: DistributionPoint) -> bool:
        ...
        if not dp.full_name and not dp.crl_issuer:
            self.log_error(
                "A DistributionPoint must contain either a distributionPoint or cRLIssuer field."
            )
            result = False

        if dp.crl_issuer and not dp.crl_issuer[0].value:
            self.log_error(
                "cRLIssuer field is present but does not contain a valid distinguished name."
            )
            result = False
...
class InhibitAnyPolicyValidation(Validation):
    ...
    def validate(self, cert: Certificate) -> bool:
        ...
        try:
            ...
            try:
                inhibit_any_policy_ext = cert.extensions.get_extension_for_oid(
            ...
            if not inhibit_any_policy_ext.critical:
                self.log_error(
                    "Inhibit Any Policy extension must be marked as critical, as required by RFC 5280."
                )
                result = False
            ...
            self.log_error(
                f"Unexpected error during Inhibit Any Policy validation: {e}"
            )
            result = False
...
class FreshestCRLValidation(Validation):
    ...
    def _get_freshest_crl_extension(
        self, cert: Certificate
    ) -> Extension[FreshestCRL] | None:
        ...
        try:
            return cast(
                ...
                cert.extensions.get_extension_for_oid(ExtensionOID.FRESHEST_CRL),
            )
        except Exception:  # noqa: BLE001
            self.log_warning(
                "Freshest CRL extension is missing. Conforming CAs are encouraged to include it."
            )
            return None

    def _validate_extension_criticality(
        self, extension: Extension[FreshestCRL]
    ) -> bool:
        ...
        if extension.critical:
            self.log_error(
                "Freshest CRL extension must be marked as non-critical, as required by RFC 5280."
            )
            return False
        return True
    ...
    def _validate_distribution_point_uris(self, dp: DistributionPoint) -> bool:
        ...
        for name in dp.full_name:
            if isinstance(name, UniformResourceIdentifier):
                ...
                if not self._is_valid_uri(uri):
                    self.log_error(
                        f"Invalid URI in distributionPoint of Freshest CRL extension: {uri}"
                    )
                    result = False
        return result
...
class SubjectAttributesValidation(Validation):
    ...
    def validate(self, cert: Certificate, *, strict: bool = True) -> bool:
        ...
        try:
            ...
            self.unmatched_patterns, self.unmatched_values = self._initialize_unmatched(
                subject_dict
            )

            result &= self._match_required_patterns(
                subject_dict, self.unmatched_patterns, self.unmatched_values
            )
            ...
            result &= self._report_unmatched(
                self.unmatched_patterns, self.unmatched_values
            )

        except Exception as e:  # noqa: BLE001  # noqa: BLE001
            self.log_error(
                f"Unexpected error during Subject Attributes validation: {e}"
            )
            result = False
    ...
    def _initialize_unmatched(
        ...
        """
        unmatched_patterns = {
            oid: set(patterns) for oid, patterns in self.required.items()
        }
        unmatched_values = {
            oid: set(subject_dict.get(oid, [])) for oid in self.required
        }
    ...
    def _match_optional_patterns(
        ...
        for oid, optional_patterns in self.optional.items():
            ...
            for value in list(unmatched_values[oid]):
                if any(
                    self._match_value_or_regex(value, pattern)
    ...
    def _handle_unexpected_attributes(
        self, subject_dict: dict[str, list[str]], *, strict: bool
    ) -> bool:
        ...
        for oid, attr_values in subject_dict.items():
            if oid not in self.required and oid not in self.optional and strict:
                self.log_error(
                    f"Unexpected attribute with OID {oid} and values {attr_values}."
                )
                result = False
    ...
    def _report_unmatched(
        ...
        for oid, patterns in unmatched_patterns.items():
            if patterns:
                self.log_error(
                    f"Required attribute with OID {oid} is missing patterns: {list(patterns)}."
                )
                result = False
        ...
        for oid, values in unmatched_values.items():
            if values:
                self.log_error(
                    f"Attribute with OID {oid} has unmatched values: {list(values)}."
                )
                result = False
    ...
    def _convert_subject_to_dict(self, subject: Name) -> dict[str, list[str]]:
        ...
        for relative_distinguished_name in subject:
            ...
            if oid not in subject_dict:
                subject_dict[oid] = []
            subject_dict[oid].append(
                value.decode("utf-8") if isinstance(value, bytes) else value
            )
        return subject_dict

    def _match_value_or_regex(
        self, attr_value: str, pattern: str | re.Pattern[str]
    ) -> bool:
...
class SANAttributesValidation(Validation):
    ...
    def _validate_required_sans(self) -> bool:
        ...
        for oid, required_patterns in self.required.items():
            ...
            if self.unmatched_patterns[oid]:
                self.log_error(
                    f"Required SAN attribute {oid} is missing patterns: {self.unmatched_patterns[oid]}."
                )
                is_valid = False
    ...
    def _validate_optional_sans(self) -> bool:
        ...
        for oid, optional_patterns in self.optional.items():
            ...
            self.unmatched_values[oid] = [
                v for v in self.unmatched_values[oid] if v not in optional_matches
            ]
    ...
    def _check_unexpected_sans(self) -> bool:
        ...
        for oid, san_values in self.san_dict.items():
            if oid not in self.required and oid not in self.optional:
                self.log_error(
                    f"Unexpected SAN attribute {oid} and values {san_values}."
                )
                is_valid = False
        return is_valid

    def _convert_san_to_dict(
        self, san_list: Sequence[GeneralName]
    ) -> dict[str, list[str]]:
        ...
        for san in san_list:
            if isinstance(san, RFC822Name):
                ...
                error_message = (
                    f"Unknown SAN type encountered: {san.__class__.__name__}"
                )
