...
def test_archiver_creates_non_empty_archive(archive_format):
    ...
    archive_bytes = Archiver.archive(
        data_to_archive=SAMPLE_DATA, archive_format=archive_format
    )
...
def test_archiver_archive_integrity(archive_format, tmp_path):
    ...
    for file_name, expected_content in SAMPLE_DATA.items():
        ...
        assert extracted_file.read_bytes() == expected_content, (
            f"Content mismatch for file {file_name}."
        )
