...
from trustpoint_core.serializer.credential import (
...
def test_init_from_bytes_pkcs12(pkcs12_data: bytes) -> None:
    ...
    assert isinstance(
        serializer.additional_certificates, CertificateCollectionSerializer
    )
...
def test_init_from_credential_serializer(pkcs12_data: bytes) -> None:
    ...
    assert (
        clone.credential_certificate.as_pem()
        == original.credential_certificate.as_pem()
    )
    assert len(clone.additional_certificates) == len(original.additional_certificates)


def test_init_from_tuple_2(
...
def test_from_crypto_pkcs12_direct(pkcs12_data: bytes) -> None:
    ...
    pkcs12_obj = pkcs12.load_pkcs12(pkcs12_data, None)
    priv_key, cert_ser, certs_coll = CredentialSerializer._from_crypto_pkcs12(
        pkcs12_obj
    )
...
def test_from_bytes_pkcs12_direct(pkcs12_data: bytes) -> None:
    ...
    priv_key, cert_ser, certs_coll = CredentialSerializer._from_bytes_pkcs12(
        pkcs12_data, None
    )
    assert isinstance(priv_key, PrivateKeySerializer)
    assert isinstance(cert_ser, CertificateSerializer)
...
def test_len_without_additional_certs(
    rsa_private_key: RSAPrivateKey, self_signed_cert: Certificate
) -> None:
...
def test_get_as_separate_pem_files_pkcs8(
    rsa_private_key: RSAPrivateKey, self_signed_cert: Certificate
) -> None:
...
def test_get_as_separate_pem_files_pkcs1(
    rsa_private_key: RSAPrivateKey, self_signed_cert: Certificate
) -> None:
...
def test_additional_certificates_setter(
    pkcs12_data, second_self_signed_cert: Certificate
) -> None:
    """Test the additional_certificates property setter."""
    serializer = CredentialSerializer(pkcs12_data)

    new_coll = CertificateCollectionSerializer(
        [second_self_signed_cert, second_self_signed_cert]
    )
...
def test_all_certificates_no_additional(
    rsa_private_key: RSAPrivateKey, self_signed_cert: Certificate
) -> None:
...
def test_get_encryption_algorithm() -> None:
    ...
    algo_with_password = CredentialSerializer._get_encryption_algorithm(
        password=b"secret"
    )
...
def test_load_pkcs12_valid(pkcs12_data: bytes) -> None:
    ...
    key, cert, additional_certs = CredentialSerializer._load_pkcs12(
        pkcs12_data, password=None
    )
